...
@section('title', 'Tambah Gedung Baru')
@section('header', 'Tambah Gedung Baru')

@section('content')
    <div class="container mx-auto">
        <div class="bg-white p-8 rounded-lg shadow-md max-w-lg mx-auto">
            <h2 class="text-2xl font-bold mb-6 text-gray-800">Formulir Gedung</h2>

            <form action="{{ route('admin.buildings.store') }}" method="POST" class="space-y-6" enctype="multipart/form-data">
                @csrf
                <div>
                    ...
                    <div class="mt-1">
                        <x-input type="text" name="name" id="name" value="{{ old('name') }}"
                            placeholder="Contoh: Gedung Utama" required />
                        @error('name')
                ...
                <div>
                    <x-file-input name="image" label="Gambar Gedung" />
                    @error('image')
                        <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                    @enderror
                </div>
